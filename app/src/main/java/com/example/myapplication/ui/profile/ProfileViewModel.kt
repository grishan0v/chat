...
class ProfileViewModel(private val myUserID: String, private val userID: String) : BaseViewModel() {
    ...
    private fun setupProfile() {
        repository.loadUser(userID) { result: MyResult<User> ->
            onResult(_otherUser, result)
            if (result is MyResult.Success) {
                repository.loadAndObserveUser(myUserID, firebaseReferenceObserver) { result2: MyResult<User> ->
